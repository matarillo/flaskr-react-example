...
メモ化が効果を発揮するのは、**あるクライアント状態の変更が、その状態と無関係なコンポーネントの再計算・再レンダリングを引き起こす**場合である。典型例はクライアントサイド検索（入力のたびにフィルタ結果を再計算）、Context の値変更による広範な再レンダリング、リッチなリストアイテムの個別更新（1件の変更で全アイテムが再レンダリング）などである。

本プロジェクトではデータ取得が loader、変更が action に集約されており、クライアント側の状態は `isSubmitting` 程度に限られる。ある状態の変更が無関係なコンポーネントに波及する構造がそもそも存在しないため、メモ化を適用しても**スキップされるレンダリングがない**。これは Global State やキャッシュ層が不要であるのと同じく、loader/action パターンによるクライアント状態の最小化がもたらす帰結である。

いいね機能の追加（実験候補 #2）では `useFetcher` による個別アイテムの更新と `useOptimistic` による楽観的 UI が加わるため、`React.memo` / `useCallback` の必要性を実際に体感できる見込みである。

> **React Compiler との関係**：React Compiler（実験的機能）はコンポーネントの依存関係を静的解析し、手動メモ化と同等の最適化を自動適用することを目指している。安定化すれば `useMemo` / `React.memo` / `useCallback` の手動管理自体が不要になる方向だが、「いつメモ化が必要になるか」の判断基準はコンパイラの有無にかかわらず有効な知識である。
